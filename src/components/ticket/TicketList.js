...
const TicketList = () => {
  ...
  const fetchTickets = async () => {
    try {
      setLoading(true);
      setError('');
      const response = await fetch('https://namami-infotech.com/NIKHILOFFSET/src/support/get_tickets.php');
      ...
      <Box sx={{ mb: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 2 }}>
        ...
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} sx={{ minWidth: { xs: '100%', sm: 'auto' } }}>
          ...
          <Button
            ...
            sx={{ whiteSpace: 'nowrap', backgroundColor: '#344C7D', color: '#fff', '&:hover': { backgroundColor: '#344C7D' } }}
          >
            New Ticket
          </Button>
        </Stack>
      ...
      {loading ? (
        ...
        <>
          ...
          <TableContainer component={Paper} variant="outlined" sx={{ mb: 3 }}>
            <Table>
              <TableHead>
                <TableRow sx={{ backgroundColor: '#344C7D', color: '#fff' }}>
