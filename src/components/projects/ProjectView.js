...
const StyledTableContainer = styled(TableContainer)(({ theme }) => ({
  borderRadius: 12,
  "& .MuiTableHead-root": {
    background: "linear-gradient(135deg, #344C7D 0%, #344C7D 100%)",
  },
    ...
    const fetchData = async () => {
      try {
        ...
        const [stationsResponse, tasksResponse, employeesResponse, completionDateResponse] = await Promise.all([
          axios.get(`https://namami-infotech.com/NIKHILOFFSET/src/tender/tender_stations.php?ActivityId=${ActivityId}`),
          axios.get(`https://namami-infotech.com/NIKHILOFFSET/src/task/project_task.php?TenderNo=${tenderNo}`),
          axios.get("https://namami-infotech.com/NIKHILOFFSET/src/employee/list_employee.php?Tenent_Id=1"),
          axios.get(`https://namami-infotech.com/NIKHILOFFSET/src/task/project_complete.php?TenderNo=${tenderNo}`)
        ])
  ...
  const handleReassignTask = async () => {
    ...
    try {
      const response = await axios.post(
        "https://namami-infotech.com/NIKHILOFFSET/src/task/edit_task.php",
        {
    ...
    <Container maxWidth="xl" sx={{ py: 0 }}>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        ...
        <Box sx={{ mb: 4, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: 'wrap' }}>
          <Box>
            <Typography variant="h4" component="h1" gutterBottom sx={{ fontWeight: 700, color: "#344C7D" }}>
          ...
          <Button
            ...
            sx={{ bgcolor: "#344C7D", mt: { xs: 2, sm: 0 } }}
          >
            Refresh Data
          </Button>
        </Box>
                ...
                <CardContent>
                  <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <Box>
                      ...
                      <Typography variant="h4" component="div" sx={{ fontWeight: 700, color: "#344C7D" }}>
                        {stats.totalTasks}
                      </Typography>
                    </Box>
                    <Assignment sx={{ fontSize: 40, color: "#344C7D", opacity: 0.7 }} />
                  </Box>
                </CardContent>
              </StyledCard>
            </motion.div>
