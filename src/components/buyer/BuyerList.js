...
function BuyerList() {
  ...
  useEffect(() => {
    const fetchBuyerData = async () => {
      setLoading(true)
      try {
        const response = await axios.get("https://namami-infotech.com/NIKHILOFFSET/src/buyer/buyer_list.php")
      ...
      <Dialog open={editDialogOpen} onClose={() => setEditDialogOpen(false)} fullWidth maxWidth="md">
        ...
        <DialogActions>
          ...
          <Button
            ...
            onClick={async () => {
              try {
                const res = await axios.post("https://namami-infotech.com/NIKHILOFFSET/src/buyer/update_buyer.php", selectedBuyer)
                if (res.data.success) {
